<?php
    ...
    class BolsaPesquisaGateway {
        ...
        public function find ($id, $class = 'stdClass') {
            $sql = "SELECT * FROM bolsaPesquisa WHERE id = '$id'";
            print "$sql <br>\n";
        ...
        public function delete ($id) {
            $sql = "DELETE FROM bolsaPesquisa WHERE id = '$id'";
            print "$sql <br>\n";
        ...
        public function save($data) {
            if (empty($data->id)) {
                $id = $this->getLastId() + 1;
                $sql = "INSERT INTO bolsaPesquisa (id, nomeBolsa, descricaoBolsa, valorBolsa, areaPesquisa) VALUES ('{$id}', '{$data->nomeBolsa}', '{$data->descricaoBolsa}', '{$data->valorBolsa}', '{$data->areaPesquisa}')";
            } else {
                $sql = "UPDATE bolsaPesquisa SET nomeBolsa = '{$data->nomeBolsa}', descricaoBolsa = '{$data->descricaoBolsa}', valorBolsa = '{$data->valorBolsa}', areaPesquisa = '{$data->areaPesquisa}' WHERE id = '{$data->id}'";
            }
        ...
        public function getLastId() {
            $sql = "SELECT max(id) as max FROM bolsaPesquisa";
            $result = self::$conn->query($sql);
